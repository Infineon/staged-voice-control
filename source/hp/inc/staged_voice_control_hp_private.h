...
#define cy_svc_log_err(ret_val,format,...)  printf ("[SHP] [Err:0x%"PRIx32", Line:%d] "format" \r\n",(uint32_t)ret_val,__LINE__,##__VA_ARGS__);
#define cy_svc_log_err_on_no_isr(ret_val,format,...)  if(false == is_in_isr()) printf ("[SHP] [Err:0x%"PRIx32", Line:%d] "format" \r\n",(uint32_t)ret_val,__LINE__,##__VA_ARGS__);
...
#define cy_svc_log_err(ret_val,format,...)  cy_log_msg (CYLF_MIDDLEWARE,CY_LOG_INFO,"[SHP] [Err:0x%"PRIx32", Line:%d] "format" \r\n",(uint32_t)ret_val,__LINE__,##__VA_ARGS__);
#define cy_svc_log_err_on_no_isr(ret_val,format,...)  if(false == is_in_isr()) cy_log_msg (CYLF_MIDDLEWARE,CY_LOG_INFO,"[SHP] [Err:0x%"PRIx32", Line:%d] "format" \r\n",(uint32_t)ret_val,__LINE__,##__VA_ARGS__);
