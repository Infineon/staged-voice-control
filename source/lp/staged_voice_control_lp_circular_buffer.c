...
#define CBUF_GUARD_SIZE_IN_BYTES (MAX_CBUF_GUARD_BYTES)

#define SAVE_CBUF_GUARD_BYTES(__X__,__Y__)  *((uint32_t *)(__X__)) = 0x1234; \
                                            *((uint32_t *)((char*)__X__+__Y__- 4)) = 0x5678;
...
#define CBUF_GUARD_SIZE_IN_BYTES (0)

#endif

#ifdef ENABLE_CBUF_SHARE_ACCCESS_TRACKER
#define ADDITIONAL_CBUF_SHARE_ACCESS_TRACKER (500)
#else
#define ADDITIONAL_CBUF_SHARE_ACCESS_TRACKER (0)
#endif
 ...
 ******************************************************************************/
#if 0
#define MAX_SHARED_BUF_SIZE (SVC_SHARED_BUFFER_SIZE + CBUF_GUARD_SIZE_IN_BYTES + ADDITIONAL_CBUF_SHARE_ACCESS_TRACKER)

#ifdef COMPONENT_CM33
#if SVC_SHARED_BUFFER_SECTION==1
#ifdef SVC_SHARED_BUFFER_SIZE
unsigned char svc_data_shared_socmem[MAX_SHARED_BUF_SIZE] __attribute__((section(".cy_shared_socmem")));
#else
#error "SVC_SHARED_BUFFER_SIZE is not defined"
#endif
#else
#error "SVC_SHARED_BUFFER_SECTION is not defined/not supported"
#endif
#endif

#ifdef COMPONENT_CM4
#if SVC_SHARED_BUFFER_SECTION==2
#ifdef SVC_SHARED_BUFFER_SIZE
unsigned char svc_data_shared_socmem[MAX_SHARED_BUF_SIZE] = {0};
#else
#error "SVC_SHARED_BUFFER_SIZE is not defined"
#endif
#else
...
void *svc_lp_allocate_buffer_to_share_between_cores(unsigned int size_of_memory)
{
    if (svc_alloc_memory)
    {
        char *p_temp = NULL;
        svc_alloc_memory(CY_SVC_MEM_ID_INTERCORE_SHARED_BUFFER,
                (uint32_t) size_of_memory, (void**) &p_temp);
        if (NULL != p_temp)
        {
            memset(p_temp, 0, size_of_memory);
            return p_temp;
        }
    }
    else
...
#if 0
#ifdef SVC_SHARED_BUFFER_SIZE
    	return (void *)&svc_data_shared_socmem[0];
#endif
#endif
    }
    return NULL;
        ...
        cy_svc_lp_config_t *create_domain)
{
    ...
    unsigned int ui_tracker_size = 0;
    unsigned int ui_svc_shared_buf_size = create_domain->inter_core_shared_buffer_size;

    if (svc_alloc_memory)
    {
        svc_alloc_memory(CY_SVC_MEM_ID_INTERNAL_BUF_HDR,
                (uint32_t) sizeof(svc_circular_buffer_header_t),
                (void**) &lp_instance->circular_shared_buffer);

        if (NULL != lp_instance->circular_shared_buffer)
        {
            memset(lp_instance->circular_shared_buffer, 0,
...
#ifdef ENABLE_CBUF_SHARE_ACCCESS_TRACKER
    ui_tracker_size = ui_svc_shared_buf_size
    ...
    lp_instance->circular_shared_buffer->original_allocated_buffer_size =
            ui_svc_shared_buf_size +
            CBUF_GUARD_SIZE_IN_BYTES + ui_tracker_size;

    lp_instance->circular_shared_buffer->original_allocated_buffer_address =
            svc_lp_allocate_buffer_to_share_between_cores(
                    lp_instance->circular_shared_buffer->original_allocated_buffer_size);

    if(NULL == lp_instance->circular_shared_buffer->original_allocated_buffer_address)
    {
        ret_val = CY_RSLT_SVC_OUT_OF_MEMORY;
        cy_svc_log_err(ret_val, "Buffer Allocate fail");
        goto CLEAN_RETURN;
    }
...
#ifdef ENABLE_CBUF_SHARE_ACCCESS_TRACKER
    lp_instance->circular_shared_buffer->buffer_share_access_tracker =
            lp_instance->circular_shared_buffer->original_allocated_buffer_address
                    + (CBUF_GUARD_SIZE_IN_BYTES / 2);
    ;
    lp_instance->circular_shared_buffer->buffer_share_access_tracker_size =
            ui_tracker_size;
    memset(lp_instance->circular_shared_buffer->buffer_share_access_tracker, 0,
            ui_tracker_size);
#else
    lp_instance->circular_shared_buffer->buffer_share_access_tracker = NULL;
    lp_instance->circular_shared_buffer->buffer_share_access_tracker_size = 0;
#endif
    ...
    lp_instance->circular_shared_buffer->frame_size_in_bytes = FRAME_DATA_SIZE(
            create_domain->audio_input_type, SAMPLE_SIZE_IN_BYTES,
            create_domain->sample_rate*1000,
            create_domain->single_frame_time_ms);
    ...
    cy_svc_log_dbg(
            "CBufHDR [Addr:%p] [Atype:%d,FrSz:%d,SaSz:%d,Wo:%d,Ro:%d]",
            lp_instance->circular_shared_buffer,
            lp_instance->circular_shared_buffer->audio_type,
    ...
    cy_svc_log_dbg(
            "CBufHDR [Details: %p:%d = [Guard:%p:%d + Track:%p:%d + buf:%p:%d]",
            lp_instance->circular_shared_buffer->original_allocated_buffer_address,
            lp_instance->circular_shared_buffer->original_allocated_buffer_size,
            lp_instance->circular_shared_buffer->actual_buffer_start_address_with_guard,
            CBUF_GUARD_SIZE_IN_BYTES,
            lp_instance->circular_shared_buffer->buffer_share_access_tracker,
            lp_instance->circular_shared_buffer->buffer_share_access_tracker_size,
            lp_instance->circular_shared_buffer->buffer_start_address,
            lp_instance->circular_shared_buffer->circular_buf_size);
    ...
    if (CY_RSLT_SUCCESS != ret_val)
    {
        if (NULL != lp_instance->circular_shared_buffer)
        {
                    ...
                    != lp_instance->circular_shared_buffer->original_allocated_buffer_address)
            {
                if (svc_free_memory)
                {
                    svc_free_memory(CY_SVC_MEM_ID_INTERCORE_SHARED_BUFFER,
                            lp_instance->circular_shared_buffer->original_allocated_buffer_address);
                }
                else
                {
                    /* Free is not required, as the memory is statically allocated */
                    free(
                            lp_instance->circular_shared_buffer->original_allocated_buffer_address);
                }
            ...
            if (svc_free_memory)
            {
                svc_free_memory(CY_SVC_MEM_ID_INTERNAL_BUF_HDR,
                        lp_instance->circular_shared_buffer);
            }
            else
            {
        ...
        svc_lp_instance_t *lp_instance)
{
    ...
    if(NULL != lp_instance->circular_shared_buffer)
    {
        /* This will be statically allocated and it will be never be freed. */
    	if(NULL != lp_instance->circular_shared_buffer->original_allocated_buffer_address)
    	{
            if (svc_free_memory)
            {
    		...
    		lp_instance->circular_shared_buffer->original_allocated_buffer_address = NULL;
	    ...
	    if(svc_free_memory)
	    {
	    	svc_free_memory(CY_SVC_MEM_ID_INTERNAL_BUF_HDR,lp_instance->circular_shared_buffer);
	    }
	    else
	    {
	    	free(lp_instance->circular_shared_buffer);
...
extern unsigned int sod_detect_delay_look_back_frame_counter;
        ...
        uint16_t *insuff_frame_counter)
{
    ...
    unsigned int number_frames_before_trigger = 0;
    unsigned int frame_counter_received_after_last_aad_dbg_for_lpwwd = 0;

    *insuff_frame_counter = 0;

    if(SVC_TRIGGER_LOW_POWER_WAKEUP_WORD_DETECTED == stage_trigger)
    {
    	frame_counter_received_after_last_aad_dbg_for_lpwwd =
    			lp_instance->stats.frame_counter_received_after_last_aad_dbg;

    	if(sod_detect_delay_look_back_frame_counter > 0 )
    	{
    		frame_counter_received_after_last_aad_dbg_for_lpwwd = frame_counter_received_after_last_aad_dbg_for_lpwwd -
    				sod_detect_delay_look_back_frame_counter;
    	}

        if (frame_counter_received_after_last_aad_dbg_for_lpwwd
        ...
        {
            if(frame_counter_received_after_last_aad_dbg_for_lpwwd > 0)
            {
                number_frames_before_trigger =
                     frame_counter_received_after_last_aad_dbg_for_lpwwd - 1;
...
{
    cy_rslt_t ret_val = CY_RSLT_SVC_GENERIC_ERROR;
    unsigned int data_to_send_in_bytes = 0;
    unsigned int total_data_sent_in_bytes = 0;
    ...
    else if (SVC_TRIGGER_SEND_POST_WWD_HPWWD_DET_IN_PROGRESS == stage_trigger)
    {
                ...
                & lp_instance->init_params.stage_config_list)
        {
            ...
            cy_svc_log_info("LastSentPostWWDAddress:%p (next new frame)",
                    lp_instance->hpwwd_trigger_data_final_address);
            ...
            != lp_instance->circular_shared_buffer->buffer_share_access_tracker)
    {
                ...
                >= lp_instance->circular_shared_buffer->buffer_start_address)
        {
            frame_offset_from_buffer_start =
                    ((char*) data_pointer
                            - (char*) lp_instance->circular_shared_buffer->buffer_start_address)
                            / lp_instance->circular_shared_buffer->frame_size_in_bytes;

            if (*(lp_instance->circular_shared_buffer->buffer_share_access_tracker
                    + frame_offset_from_buffer_start) == 1)
            {
                ret_val = CY_RSLT_SVC_DATA_OVERWRITE_WARNING;
                cy_svc_log_err_on_no_isr(ret_val, "CBUF-MARK: DATA overwrite warning (CORE2 not processed) !!!");
                //	printf("CBUF-MARK: DATA overwrite warning (CORE2 not processed) !!!\r\n");
                return ret_val;
            }
            else
            {
        ...
        {
        	cy_svc_log_err_on_no_isr(ret_val, "CBUF-MARK: Invalid frame buffer");
        }
    }
    else
    {
...
//                cy_svc_log_info("CBUF-MARK-SET: Offset:%d, Count:%d",
//                        frame_offset_from_buffer_start, frame_count);
            ...
            {
                memset(
                        ((char*) lp_instance->circular_shared_buffer->buffer_share_access_tracker
                                + frame_offset_from_buffer_start), 1,
                        frame_count);
...
//                        frame_offset_from_buffer_start, frame_count);
            ...
            {
                cy_svc_log_err(ret_val, "CBUF-MARK-SET: Invalid frame count");
            }
        }
        else
        {
            cy_svc_log_err(ret_val, "CBUF-MARK-SET: Invalid frame buffer");
        }
    }
    else
    {
        cy_svc_log_info("CBUF-MARK-SET: Buffer access marker disabled");
    }
    return ret_val;
}
#endif
