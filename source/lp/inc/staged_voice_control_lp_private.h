...
#define cy_svc_log_err(ret_val,format,...)  printf ("[SLP] [Err:0x%x, %s:%d] "format" \r\n",(unsigned int)ret_val,__FUNCTION__,__LINE__,##__VA_ARGS__);
#define cy_svc_log_err_on_no_isr(ret_val,format,...)  if(false == is_in_isr()) printf ("[SLP] [Err:0x%x, %s:%d] "format" \r\n",(unsigned int)ret_val,__FUNCTION__,__LINE__,##__VA_ARGS__);
...
#define cy_svc_log_err(ret_val,format,...)  cy_log_msg (CYLF_MIDDLEWARE,CY_LOG_INFO,"[SLP] [Err:0x%x, %s:%d] "format" \r\n",(unsigned int)ret_val,__FUNCTION__,__LINE__,##__VA_ARGS__);
#define cy_svc_log_err_on_no_isr(ret_val,format,...)  if(false == is_in_isr()) cy_log_msg (CYLF_MIDDLEWARE,CY_LOG_INFO,"[SLP] [Err:0x%x, %s:%d] "format" \r\n",(unsigned int)ret_val,__FUNCTION__,__LINE__,##__VA_ARGS__);
