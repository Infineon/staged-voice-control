...
#define cy_lpwwd_prewwd_log_err(ret_val,format,...)  printf ("[PREWWD] [Err:0x%lx, %s:%d] "format" \r\n",ret_val,__FUNCTION__,__LINE__,##__VA_ARGS__);
...
#define cy_lpwwd_prewwd_log_err(ret_val,format,...)  cy_log_msg (CYLF_MIDDLEWARE,CY_LOG_INFO,"[PREWWD] [Err:0x%lx, %s:%d] "format" \r\n",ret_val,__FUNCTION__,__LINE__,##__VA_ARGS__);
